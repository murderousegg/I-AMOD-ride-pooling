def LTIFM2_SP_dests(jj1, ii1, jj2, ii2, solPart, node_order, dest_order):
    '''
    Helper function that computes delay experienced by 2 OD-pairs when combined.
    Returns the nodes respective to the input indices.

    Parameters:
    ----------
    jj1: origin 1 (as array index)
    ii1: destination 1 (as array index)
    jj2: origin 2 (as array index)
    ii2: destination 2 (as array index)
    solPart: Precomputed shortest route costs
    node_order: list containing node name corresponding to array index
    dest_order: list containing node name corresponding to array index from destinations
    '''
    sol1 = {}
