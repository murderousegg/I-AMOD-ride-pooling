...
def A1_SP():
    ...
    delayed(compute_sol_row)(ii, G_roadgraph, car_node_order)
...
def compute_LinearComb2_for_jj1(jj1, N_nodes, solPart):
    ...
    """
    # sol2_LC = np.zeros([10*N_nodes*N_nodes,11]);  
    counter=0
        ...
        for ii2 in range(ii1,N_nodes):
            for jj2 in range(jj1,N_nodes):
                ...
                if not np.any([car_node_order[ii2] == car_node_order[jj2], car_node_order[ii1] == car_node_order[jj1]]):
                    ...
                    if solPart[jj1, ii1]["obj"] >= 4/60 and solPart[jj2, ii2]["obj"] >= 4/60:
                        ...
                        if opti[0,0] != 0 and opti[0,2]<=0.1 and opti[0,1]<=0.1:
                            counter = counter+1
...
def A2_LinearComb2(solPart):
    ...
    sol2_LC_arr = sol2_LC_arr[sol2_LC_arr[:, 0] < -0.01]
