...
def IAMoD_loop(FullList, exogenous_G=False, pen_rate=1):
    ...
    for iamod_iter in range(max_iter):
        ...
        if iamod_iter == 2:
            mu = 0.0001
        _,_,_,x,expected_cars, obj = cars.solve_bush_CARSn(tnet=tNet, fcoeffs=fcoeffs, iteration=iamod_iter, unf=False, r=r, mu=mu, prev_x=prev_x, reb_cars=reb_cars, c_ratio=c_ratio, vehicle_lim=VEHICLE_LIM*pen_rate, server=True)
...
def run_scenario(pen_rate, FullList):
    ...
    pedFlow = sum([tNet.G_supergraph[i][j]['flow'] * tNet.G_supergraph[i][j]['t_0'] for i,j in pedestrian])
    bikeFlow = sum([tNet.G_supergraph[i][j]['flow'] * tNet.G_supergraph[i][j]['t_0'] for i,j in bike])
    solo_ride = total_PercNRP[0]*sum([tNet.G_supergraph[i][j]['flow'] * tNet.G_supergraph[i][j]['t_1'] for i,j in ridepool])
    shared_ride = total_PercNRP[1]*sum([tNet.G_supergraph[i][j]['flow'] * tNet.G_supergraph[i][j]['t_1'] for i,j in ridepool])
    privateFlow = sum([tNet_private.G[i][j]['flow'] * tNet_cars.G[i][j]['t_1'] for i,j in tNet_private.G.edges()])
    rebFlow = sum([tNet_cars.G[i][j]['flowRebalancing'] * tNet_cars.G[i][j]['t_1'] for i,j in tNet_cars.G.edges()])
    IAMoDFlow = sum([tNet.G_supergraph[i][j]['flow'] * tNet.G_supergraph[i][j]['t_1'] for i,j in tNet.G_supergraph.edges()])
    ### append costs
    IAMoDCosts = (IAMoDFlow/tNet.totalDemand)
    privateCosts = (privateFlow[-1]/tNet_private.totalDemand)
    totCost = ((IAMoDFlow+privateFlow[-1])/(tNet.totalDemand+tNet_private.totalDemand))  #TODO: calculate the cost with rebalancing
    return dict(
        pen_rate     = pen_rate,
        IAMoDCost    = IAMoDCosts,
        privateCost  = privateCosts,
        totalCost    = totCost,
        flows        = (pedFlow, bikeFlow, solo_ride, shared_ride,
                        privateFlow, rebFlow, IAMoDFlow)
    )
...
def main():
    ...
    bikeFlow = []
    rebFlow = []
    pen_space = np.linspace(0.01,0.99, 10)
    with Parallel(n_jobs=6, backend="loky") as P:
        results = P(delayed(run_scenario)(p, FullList) for p in pen_space)
    ### unpack results
    IAMoDCosts   = [r['IAMoDCost']   for r in results]
    privateCosts = [r['privateCost'] for r in results]
    totCosts     = [r['totalCost']   for r in results]

    pedFlow, bikeFlow, solo_ride, shared_ride, privateFlow, rebFlow, IAMoDFlow = map(
        list, zip(*(r['flows'] for r in results))
    )    
    ...
    plt.figure()
    plt.plot(list(np.linspace(0.01,0.99, len(totCosts))), totCosts, label='Average all users', marker='o')
    plt.plot(list(np.linspace(0.01,0.99, len(totCosts))), IAMoDCosts, label='I-AMoD', marker='^')
    plt.plot(list(np.linspace(0.01,0.99, len(totCosts))), privateCosts, label='Private', marker= 'x')
    plt.legend()
    ...
    plt.figure()
    width = 0.05
    ind = list(np.linspace(0.01,0.99, len(totCosts)))
    ...
    save_results(results=(pedFlow, bikeFlow, solo_ride, shared_ride, privateFlow, rebFlow, IAMoDCosts, privateCosts, totCosts), dir_out=dir_out)
