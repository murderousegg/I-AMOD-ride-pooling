...
from src.LTIFM_reb import LTIFM_reb, LTIFM_reb_sparse
...
def compute_results(
    ...
    del delay1, delay2, o1_raw, d1_raw, o2_raw, d2_raw,\
          pattern, lut, jj1_arr, jj2_arr, ii1_arr, ii2_arr
    gc.collect()
...
class SimulationConfig:
    ...
    def results_csv(self) -> Path:
        ...
        return self.results_dir / f"results_penrate_{self.city_tag}"
...
class RidePoolingSimulation:
    ...
    def run(self, penrate) -> None:
        ...
        self._save_metrics_csv(penrate)

    def run_private(self, penrate) -> None:
    ...
    def _initialize_networks(self, pen_rate):
        ...
        self.tNet_private.set_g(tnet.perturbDemandConstant(self.g, constant=pen_rate))
    ...
    def _save_metrics_csv(self, penrate: float) -> None:
        df = pd.DataFrame(self.metrics)
        df.to_csv(self.cfg.results_csv + f"_{penrate}.csv", index=False)
        logger.info("Metrics saved → %s", self.cfg.results_csv)
...
def main() -> None:
    ...
    for pen_rate in np.linspace(0.01,0.99, 10):
        for stackelberg in range(10):
            ...
            sim.run(pen_rate)
            sim.run_private(pen_rate)
        sim.log_penrate_results()
