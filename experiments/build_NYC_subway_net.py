...
def build_NYC_net(dir, only_road=False, symbls=False):
    ...
    if only_road == False:
        ...
        for filename in os.listdir(dir):
            if filename.endswith(".txt") and not filename.endswith('Road_net.txt') and not filename.endswith('Subway_net.txt'):
                netFile = os.path.join(dir, filename)
...
# tNet, tstamp, fcoeffs = build_NYC_net('data/net/NYC/')
# print(tNet.G_supergraph.nodes())
