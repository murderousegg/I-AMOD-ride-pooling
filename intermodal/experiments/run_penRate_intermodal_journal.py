...
def solve_stackelberg_game(par):
    ...
    if "B" in str(sys.argv[3]):
        #one_way was set to True?
        tNet_cavs.build_layer(one_way=False, avg_speed=6,capacity=99999, symb="b", identical_G=False)
        tNet_non_cavs.build_layer(one_way=False, avg_speed=6, capacity=99999, symb="b", identical_G=False)
    ...
    if "P" in str(sys.argv[3]):
        ...
        tNet_non_cavs.build_layer(one_way=False, avg_speed=3.1, capacity=9999, symb="'", identical_G=False)
    # add subway for nyc
    if netname == 'NYC' and "S" in str(sys.argv[3]):
        ...
        tNet_non_cavs.add_layer(layer=layer, layer_symb='s'\
                                # , speed=0.001\
                                )
...
def penRate(netname, dir_out, rebalancing=True, linear=False, n=5, theta_n=3, demand_multiplier=1,n_iter = 5,  parallel=True):
    ...
    subwayFlow = []
    if parallel:
        pool = mp.Pool(min(4,len(PENETRATION_RATES)))
        pars = [(netname, dir_out, penetration_rate, rebalancing, linear, n, theta_n, demand_multiplier, n_iter) for penetration_rate in PENETRATION_RATES]
...
def plot_penRate(result, dir_out):
    ...
    lstyle = ['-', '--', ':']
    i = 0    
    ax1.plot(PENETRATION_RATES, totCost, label=alg + ' (Total)', color='green', linestyle=lstyle[i], linewidth=2, marker='o')
    ax1.plot(PENETRATION_RATES, cavsCost, label=alg + ' (AMoDs)', color='blue', linestyle=lstyle[i], linewidth=2, marker="^")
    ax1.plot(PENETRATION_RATES, noncavsCost, label=alg + ' (Private)', color='red', linestyle=lstyle[i], linewidth=2, marker='x')
    ax1.legend()
    ...
    width = 0.9
    x_name = [round(.1 * i, 1) for i in range(len(PENETRATION_RATES))]
